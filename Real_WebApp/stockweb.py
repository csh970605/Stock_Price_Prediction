...
class WebApplication :
    def __init__(self):
        ...
        image = Image.open('C:/StockWebApp/사용방법.png')
        st.image(image)

        st.sidebar.header('유저 설정')
        st.sidebar.text("\n")

        self.Picompname = ['하이스틸', 'POSCO', '영흥', '부국철강', 'DSR제강', 'NI스틸', '고려제강', '한일철강', '대한제강', 'KG동부제철우',
        ...
        self.Dakcompname = ['포스코엠텍', '유에스티', '금강철강', '동양에스텍', '삼현철강', '대창스틸', '대호피앤씨', '대동스틸',
                       ...
                       '유안타제6호스팩', 'IBKS제15호스팩', '케이비제20호스팩', '하나금융16호스팩', '한국9호스팩', '케이프이에스제4호', 'IBK제13호스팩',
                       'DB금융스팩8호',
                       ...
                       '에이프로', '세코닉스', '바이오스마트', '라온피플', '카스', '아이엠', '인터엠', '비츠로셀', '상아프론테크', '성호전자', '에스씨디',
                       '디에이테크놀로지',
    ...
    def get_input(self):
        self.start_date = st.sidebar.date_input("시작일")
        self.end_date = st.sidebar.date_input("종료일", self.start_date + timedelta(days=90))
        self.kospidak = st.sidebar.selectbox("코스피/코스닥", options=self.kosPD)

        if self.kospidak == "코스피":
            self.stock_symbol = st.sidebar.selectbox("종목", options=self.Picompname)
        elif self.kospidak == "코스닥":
            self.stock_symbol = st.sidebar.selectbox("종목", options=self.Dakcompname)

        result = st.sidebar.button("예측시작")
        # self.get_company_name(symbol.upper())
        if result:
            #company_name = self.stock_symbol
            st.header(self.stock_symbol + "의 거래량\n")
            st.sidebar.text("\n")
            st.sidebar.text("\n")
            st.sidebar.text("\n")
